...
program SM_Drought_Index
  ...
  call ReadDataMain( SMI, do_cluster, ext_smi, invert_smi, &
       read_opt_h, silverman_h, opt_h, lats_1d, lons_1d, lats_2d, lons_2d, do_basin, &
  ...
  if (invert_smi) then
     ...
     call WriteNetcdf(outpath, 2, per_smi, lats_1d, lons_1d, lats_2d, lons_2d, SM_invert=dummy_D3_sp)
     deallocate(dummy_D3_sp)
  end if

  ! write output
  if (.NOT. ext_smi) then
     call WriteSMI( outpath, SMI, mask, per_eval, lats_1d, lons_1d, lats_2d, lons_2d)
    call message('write SMI...ok')
  end if

  if ((.not. read_opt_h) .and. (.not. ext_smi)) then
     call WriteCDF( outpath, SM_kde, opt_h, mask, per_kde, nCalendarStepsYear, lats_1d, lons_1d, lats_2d, lons_2d)
  ...
  if ( do_cluster ) then
     ...
     call WriteNetCDF(outpath, 3, per_smi, lats_1d, lons_1d, lats_2d, lons_2d, SMIc=SMIc)
     ...
     call WriteNetCDF(outpath, 4, per_smi, lats_1d, lons_1d, lats_2d, lons_2d)
  ...
  if ( do_sad ) then
     do d = 1, nDurations
        ...
        call WriteNetCDF(outpath, 5, per_kde, lats_1d, lons_1d, lats_2d, lons_2d, duration=durList(d))
     end do
  end if
