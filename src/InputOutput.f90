...
contains
  ...
  !               Last Update    Sa   
  !**********************************************************************
  subroutine WriteResultsKernel(outpath, wFlag, iCell, nCells, iMonth, opt_h) 
    ...
    case (1)
       ...
       fName = trim(outpath) // '/cdf_opt_' // trim(dummy)
    ...
    case (2)
       ...
       fName = trim(outpath) // '/edf_' // trim(dummy)
