...
subroutine set_netCDF_SM
  ...
  !
  ! define output file
  fName_netCDF_Out = trim(DataPathOut)//'mean_m_SM.nc'
  !
  ! define parameters
  ...
  V(i)%att(2)%xType  = NF90_CHAR
  V(i)%att(2)%nValues= 1
  V(i)%att(2)%values = 'monthly mean SM/SMs over 3L'
  !
  V(i)%att(3)%name   = 'valid_range'
  V(i)%att(3)%xType  = NF90_FLOAT
  V(i)%att(3)%nValues= 2
  ...
  if (SMI_flag == 4) then
     ...
     globalAtt(1)%values   = 'Mean Monthly SM/SMs over Germany 7 x 7 km'
     !
     ...
     globalAtt(1)%name    = 'title'
     globalAtt(1)%values   = 'Mean Monthly SM/SMs over Germany 4 x 4 km'
     !
     globalAtt(2)%name    = 'history'
     globalAtt(2)%values   = 'mHM v4.3_block, proceceed by L. Samaniego, R. Kumar & M. Zink'
  end if
end subroutine  set_netCDF_SM
