...
CONTAINS
  ...
  subroutine ReadDataMain( SMI_in, do_cluster, ext_smi, invert_SMI, read_opt_h, silverman_h, opt_h, lats_1d, lons_1d,&
    ...
    call message("||              VERSION ", version, " (", version_date, ")")
    call message("||")
    call message("====================================================")
    call message("")
    ...
    ! read namelist
    call path_isfile(path=file_namelist, quiet_=.true., throwError_=.true.)
    open (unit=10, file=file_namelist, status='old')
      read(10, nml=mainconfig)
    close (10)
    call message(file_namelist, ' read ...ok')
