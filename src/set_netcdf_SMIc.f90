...
subroutine set_netCDF_mSMIc
  ...
  !
  ! define output file
  fName_netCDF_Out = trim(DataPathOut)//'mSMIc.nc'
  !
  ! define parameters
  ...
  V(i)%att(2)%xType  = NF90_CHAR
  V(i)%att(2)%nValues= 1
  V(i)%att(2)%values = 'monthly SMI indicator SMI < th'
  !
  V(i)%att(3)%name   = 'valid_range'
  ...
  if (SMI_flag == 4) then
     ...
     globalAtt(1)%values   = 'SMI indicator over Germany 7 x 7 km'
     !
     ...
     globalAtt(1)%name    = 'title'
     globalAtt(1)%values   = 'SMI indicator over Germany 4 x 4 km'
     !
     globalAtt(2)%name    = 'history'
     globalAtt(2)%values   = 'mHM v4.3_block, proceceed by L. Samaniego, R. Kumar & M. Zink'
  endif
end subroutine  set_netCDF_mSMIc
