...
subroutine set_netCDF_SEV(d)
  ...
  ! 
  ! define output file
  write (fName_netCDF_Out, '(i2.2)') d
  fName_netCDF_Out = trim(DataPathOut)//'sev_'//trim(fName_netCDF_Out)//'.nc'
  !
  ...
  V(i)%att(1)%nValues= 1
  if (SMI_flag == 4) then
     V(i)%att(1)%values = 'd*months since 1970-12-31 0:0:0' ! COSMO
  else if (SMI_flag == 5) then
     V(i)%att(1)%values = 'd*months since 1990-12-31 0:0:0' ! WRF-NOAH
  else
     V(i)%att(1)%values = 'd*months since 1950-12-31 0:0:0' ! mHM
  end if
  !
  V(i)%att(2)%name   = 'long_name'
  V(i)%att(2)%xType  = NF90_CHAR
  ...
  V(i)%att(2)%xType  = NF90_CHAR
  V(i)%att(2)%nValues= 1
  V(i)%att(2)%values = 'd-month severity'
  !
  V(i)%att(3)%name   = 'valid_range'
  V(i)%att(3)%xType  = NF90_FLOAT
  V(i)%att(3)%nValues= 2
  ...
  if (SMI_flag == 4) then
     ...
     globalAtt(1)%values   = 'd-month drought severity over Germany 7 x 7 km'
     !
     globalAtt(2)%name    = 'history'
     globalAtt(2)%values   = 'COSMO-IMK, proceceed by L. Samaniego'
  else if (SMI_flag == 5) then
     globalAtt(1)%name    = 'title'
     ...
     globalAtt(1)%name    = 'title'
     globalAtt(1)%values   = 'd-month drought severity over Germany 4 x 4 km'
     !
     globalAtt(2)%name    = 'history'
